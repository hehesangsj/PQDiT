...
def get_initial_codebook(training_set: torch.Tensor, k: int, epsilon: float = 1e-5) -> torch.Tensor:
    ...
    """
    b, n, d = training_set.shape

    noise = torch.empty(b, k, d).normal_(0, epsilon).to(training_set.device)
...
def pairwise_squared_distances(x: torch.Tensor, y: Optional[torch.Tensor] = None) -> torch.Tensor:
    ...
    """
    x_norm = (x ** 2).sum(2).unsqueeze(-1)

    if y is not None:
        y_norm = (y ** 2).sum(2).unsqueeze(1)
    else:
        y = x
        y_norm = x_norm.unsqueeze(1)

    dist = x_norm + y_norm - 2.0 * torch.mm(x, torch.transpose(y, 1, 2))
    return dist
...
def assign_clusters(
    ...
    for i in range(num_batches):
        ...
        batch_codes = batch_distances.argmin(dim=2)
        del batch_distances
