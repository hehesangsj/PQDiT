...
class DSDTraining(nn.Module):
    ...
    def __init__(self, model, sparsity):
        ...
        for i in range(4, len(tmp)-1):
            w, b = tmp[i], tmp[i + 1]
            if "mlp.fc" in w[0] and "weight" in w[0] and "bias" in b[0]:
                print(w[0],b[0])
                self.name.append((w[0], b[0]))
    ...
    def update_masks(self):
        ...
        for i, (w, b) in enumerate(self.layers):
            # q_w = torch.quantile(torch.abs(w), q = self.sparsity, dim=1, keepdim= True)
            q_w = torch.quantile(torch.abs(w), q = self.sparsity, keepdim= True)
            mask_w = torch.abs(w) < q_w

            q_b = torch.quantile(torch.abs(b), q = self.sparsity, dim=0, keepdim= True)
            mask_b = torch.abs(b) < q_b
...
def replace_layernorm(net):
    ...
    for child_name, child in net.named_children():
        if isinstance(child, torch.nn.LayerNorm):
            setattr(net, child_name, apex.normalization.FusedLayerNorm(child.weight.size(0)))
        else:
