...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.mask_fc1 = nn.Parameter(torch.ones_like(self.fc1.weight))
        self.mask_fc2 = nn.Parameter(torch.ones_like(self.fc2.weight))


    def forward(self, x):
        # feature1 = self.fc1(x)
        # feature2 = self.act(feature1)
        # feature2 = self.drop(feature2)
        # x = self.fc2(feature2)
        # x = self.drop(x)
        x = x @ (self.mask_fc1 * self.fc1.weight).transpose(1,0) + self.fc1.bias
        x = self.act(x)
        x = x @ (self.mask_fc2 * self.fc2.weight).transpose(1,0) + self.fc2.bias
...
class layers_scale_mlp_blocks(nn.Module):
    ...
    def __init__(self, dim, drop=0., drop_path=0., act_layer=nn.GELU,init_values=1e-4,num_patches = 196):
        ...
        self.mask_attn = nn.Parameter(torch.ones_like(self.attn.weight))
    ...
    def forward(self, x):
        y = self.norm1(x).transpose(1,2) @ (self.mask_attn *self.attn.weight).transpose(1,0) + self.attn.bias
