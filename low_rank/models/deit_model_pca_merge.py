...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.uv1 = nn.Linear(int(in_features /2), hidden_features)
        self.act = act_layer()
        # self.fc2 = nn.Linear(hidden_features, int(out_features /2))
        # self.uv2 = nn.Linear(int(out_features /2), out_features)
        self.fc2 = nn.Linear(hidden_features, out_features)

        self.drop = nn.Dropout(drop)

    def forward(self, x):
        x = self.uv1(self.fc1(x))
        x = self.act(x)
        mlp_act = self.drop(x)
        # x = self.uv2(self.fc2(mlp_act))
        x = self.fc2(mlp_act)
        x = self.drop(x)
        return x, mlp_act
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # self.qkv = nn.Linear(dim, int(dim/2), bias=qkv_bias)
        # self.uv1 = nn.Linear(int(dim/2), dim * 3, bias=qkv_bias)
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        mlp_out, mlp_act_out = self.mlp(self.norm2(x))
        x = x + self.drop_path(mlp_out)
        return x, mlp_act_out
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks:
            x, y  = blk(x)
            blk_output.append(x)
            inter_output.append(y)
        x = self.norm(x)
        return x[:, 0], inter_output, x[:, 1:]

    def forward(self, x):
        feature, inter_output, patch_output = self.forward_features(x)
        x = self.head(feature)
        return x, feature
