...
class Mlp_pac(nn.Module):
    def __init__(self, dim1, dim2):
        super().__init__()
        # ratio = 0.25
        # self.v = nn.Parameter(torch.randn(hidden_features, fc1_len))
        # self.u = nn.Parameter(torch.randn(fc1_len, hidden_features))
    ...
    def forward(self, x):
        # B,N,C = x.shape
        # y = x.flatten(0,1).transpose(0,1)
        # y = self.w @ y + self.b
        # y = y.transpose(0,1).reshape(B,N,C)
