...
class Mlp_svd(nn.Module):
    def __init__(self, in_features, out_features, ratio):
        ...
        if out_features > in_features:
            self.weight_u = nn.Parameter(torch.rand(out_features, int(in_features * ratio)))
            self.weight_v = nn.Parameter(torch.rand(in_features,int(in_features * ratio)))
        else:
            self.weight_u = nn.Parameter(torch.rand(out_features, int(out_features * ratio)))
            self.weight_v = nn.Parameter(torch.rand(in_features, int(out_features * ratio)))

        self.bias = nn.Parameter(torch.rand(out_features))

    def forward(self, x):
        x = x @ self.weight_v @ self.weight_u.t() + self.bias
        return x

class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., ratio = [1,1]):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.fc1 = Mlp_svd(in_features, hidden_features, ratio[0])
        self.act = act_layer()
        self.fc2 = Mlp_svd(hidden_features, out_features, ratio[1])
...
class layers_scale_mlp_blocks(nn.Module):
    ...
    def __init__(self, dim, drop=0., drop_path=0., act_layer=nn.GELU,init_values=1e-4,num_patches = 196, ratio= [1,1]):
        super().__init__()
        self.norm1 = Affine(dim)
        self.attn = nn.Linear(num_patches, num_patches)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = Affine(dim)
        self.mlp = Mlp(in_features=dim, hidden_features=int(4.0 * dim), act_layer=act_layer, drop=drop, ratio=ratio)
...
class resmlp_models(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,drop_rate=0.,
        ...
        #         num_patches=num_patches, ratio = block_ratio[])
        ...
        for i in range(depth):
            ratio = [1,1]
            if i * 2 in block_ind:
                ind = block_ind.index(i*2)
                ratio[0] = block_ratio[ind]
            if i * 2 + 1 in block_ind:
                ind = block_ind.index(i*2 + 1)
                ratio[1] = block_ratio[ind]
            
            block = layers_scale_mlp_blocks(
                dim=embed_dim,drop=drop_rate,drop_path=dpr[i],
                act_layer=act_layer,init_values=init_scale,
                num_patches=num_patches, ratio = ratio)
