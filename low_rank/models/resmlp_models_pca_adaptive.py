...
class layers_scale_mlp_blocks(nn.Module):
    ...
    def __init__(self, dim, drop=0., drop_path=0., act_layer=nn.GELU,init_values=1e-4,num_patches = 196, fc1_use_uv=True, fc2_use_uv=True,  fc1_len= 0, fc2_len=0):
        super().__init__()
        self.norm1 = Affine(dim)
        self.attn = nn.Linear(num_patches, num_patches)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = Affine(dim)
        self.mlp = Mlp(in_features=dim, hidden_features=int(4.0 * dim), act_layer=act_layer, drop=drop, fc1_use_uv=fc1_use_uv, fc2_use_uv=fc2_use_uv,fc1_len = fc1_len, fc2_len = fc2_len)
...
class resmlp_models(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,drop_rate=0.,
                ...
                drop_path_rate=0.0,init_scale=1e-4, fc1_use_uv=[], fc2_use_uv=[], fc1_len=[], fc2_len=[]):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            layers_scale_mlp_blocks(
                ...
                num_patches=num_patches, fc1_use_uv = fc1_use_uv[i],
                fc2_use_uv = fc2_use_uv[i],fc1_len = fc1_len[i], fc2_len = fc2_len[i] )
    ...
    def forward_features(self, x):
        ...
        feature = self.norm(x)
        x = feature.mean(dim=1).reshape(B,1,-1)
