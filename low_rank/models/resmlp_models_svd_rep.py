...
class Mlp_svd(nn.Module):
    def __init__(self, in_features, out_features):
        ...
        if out_features > in_features:
            ...
            self.weight_rep = nn.Parameter(torch.eye(int(in_features * ratio),int(in_features * ratio)))
        else:
            ...
            self.weight_rep = nn.Parameter(torch.eye(int(out_features * ratio),int(out_features * ratio)))

        self.bias = nn.Parameter(torch.rand(out_features))

    def forward(self, x):
        x = x @ self.weight_v @ self.weight_rep @ self.weight_u.t() + self.bias
        return x
