...
def sparse_module(model_dict, sparse_rate):
    new_dict = {}
    for k,v in model_dict.items():
        if  ("attn.qkv.weight" in k) or ("attn.proj.weight" in k):
            # print(k)
            sparse_num = int(v.size(1) * sparse_rate)
            w = v.clone().detach()
            if sparse_num > 0:
                q_w = torch.quantile(torch.abs(w), q = sparse_rate, keepdim= True)
                w[torch.abs(w) < q_w] = 0
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler):
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        #     new_dict = sparse_module(model_dict, 0.2)
