...
def main(args):
    ...
    if rank == 0:
        ...
        experiment_dir = f"{args.results_dir}/004-DiT-XL-2"  # Create an experiment folder
    ...
    model_pq = compress_model(model, **compression_config).cuda()
    ...
    # optimizer
    opt = torch.optim.AdamW(model_pq.parameters(), lr=1e-4, weight_decay=0)
    ...
    model_pq.train()  # important! This enables embedding dropout for classifier-free guidance
    train_steps = 0
    log_steps = 0
    running_loss = 0
    start_time = time()
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in loader:
            ...
            loss_dict = diffusion.training_losses(model_pq, x, t, model_kwargs)
            ...
            if train_steps % args.ckpt_every == 0:
                if rank == 0:
                    checkpoint = {
                        "model": model_pq.module.state_dict(),
                        "opt": opt.state_dict(),
                        "args": args
                    }
    ...
    model_pq.eval()  # important! This disables randomized embedding dropout
